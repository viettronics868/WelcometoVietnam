...
import com.khoanam.youtubevideowall.ui.FlippingView;
import com.khoanam.youtubevideowall.ui.ImageWallView;
...
public class VideoWallDemoActivity extends Activity implements
  ...
  //private static final float PLAYER_VIEW_MINIMUM_HEIGHT_DP = 110;// original
  private static final float PLAYER_VIEW_MINIMUM_HEIGHT_DP = 90;
  //private static final int MAX_NUMBER_OF_ROWS_WANTED = 4;// original
  ...
  //private static final String PLAYLIST_ID = "PLdqmy90rx_N2pMZ4_tv2zSsvId2fLjeZE";// english at work with Winx Club
  //private static final String PLAYLIST_ID = "PLdqmy90rx_N2K1l3BMffyssqB3QSXCwC1";// descendants of the sun

  private String PLAYLIST_ID;
  private static final int INTER_IMAGE_PADDING_DP = 5;

  // YouTube thumbnails have a 16 / 9 aspect ratio
  private static final double THUMBNAIL_ASPECT_RATIO = 16 / 9d; // original
  ...
  private static final String LOG_TAG="VIDEO WALL";
  ...
  private boolean pDialogShow = false;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    PLAYLIST_ID = intent.getStringExtra("Tag");
  ...
  public void onInitializationSuccess(YouTubeThumbnailView thumbnailView, YouTubeThumbnailLoader thumbnailLoader) {
    ...
    thumbnailLoader.setOnThumbnailLoadedListener(new ThumbnailListener());//original
    maybeStartDemo(); //original
  ...
  protected void onResume() {
    ...
    if ((!nextThumbnailLoaded)  // after loading all of thumbnail, it is not show progress dialog
            && (!pDialogShow) ) // after resuming, it must check any dialog existing, if any, do not show progress dialog
    {
      pDialog = new ProgressDialog(VideoWallDemoActivity.this);
  ...
  public void onFlipped(FlippingView view) {
    ...
    imageWallView.    // one step to click thumbnail and play video corresponding this thumbnail
            getChildAt(imageWallView.getElementIdx(flippingCol,flippingRow)).
            setTag(strThumbnailVideoId);

    hidePDialog();// before show thumbnail, please stop progress dialog
  ...
  private void loadNextThumbnail() {
    ...
    if (thumbnailLoader.hasNext()) {
      ...
      //Log.d(LOG_TAG, "load Next Thumbnail start");
    } else {
      thumbnailLoader.first();
      //Log.d(LOG_TAG,"load First Thumbnail start");
    }
  ...
  private final Handler mHandler = new Handler(){
    ...
    public void handleMessage(Message msg){
      ...
      if (pDialog !=null){
        pDialog.dismiss();
        Toast.makeText(VideoWallDemoActivity.this,"Please press BACK and try again",Toast.LENGTH_LONG).show();
      }
    }
  };
